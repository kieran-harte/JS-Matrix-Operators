...
module.exports = function (babel) {
	...
	var preCode = (function () {
		var _Op = (function () {
			...
			return {
				hadamard(a, b) {
					if (a.operatorHadamard) return a.operatorHadamard(b);
					if (a.constructor.operatorHadamard) return a.constructor.operatorHadamard(a,b);
					return undefined;
				},
				add(a, b) {
					if (a.operatorAdd) return a.operatorAdd(b);
					if (a.constructor.operatorAdd) return a.constructor.operatorAdd(a,b);
					else return a + b;
				},
				
				sub(a, b) {
					if (a.operatorSub) return a.operatorSub(b);
					if (a.constructor.operatorSub) return a.constructor.operatorSub(a,b);
					else return a - b;
				},
				
				mul(a, b) {
					if (a.operatorMul) return a.operatorMul(b);
					if (a.constructor.operatorMul) return a.constructor.operatorMul(a,b);
					else return a * b;
				},
				
				div(a, b) {
					if (a.operatorDiv) return a.operatorDiv(b);
					if (a.constructor.operatorDiv) return a.constructor.operatorDiv(a,b);
					else return a / b;
				},
				
				pow(a, b) {
					if (a.operatorPow) return a.operatorPow(b);
					if (a.constructor.operatorPow) return a.constructor.operatorPow(a,b);
					else return a ** b;
				},
				
				binaryAnd(a, b) {
					if (a.operatorBinaryAnd) return a.operatorBinaryAnd(b);
					if (a.constructor.operatorBinaryAnd) return a.constructor.operatorBinaryAnd(a,b);
					else return a & b;
				},
				
				binaryOr(a, b) {
					if (a.operatorBinaryOr) return a.operatorBinaryOr(b);
					if (a.constructor.operatorBinaryOr) return a.constructor.operatorBinaryOr(a,b);
					else return a | b;
				},

				binaryXor(a, b) {
					if (a.operatorBinaryXor) return a.operatorBinaryXor(b);
					if (a.constructor.operatorBinaryXor) return a.constructor.operatorBinaryXor(a,b);
					else return a ^ b;
				},

				binaryLShift(a, b) {
					if (a.operatorBinaryLShift) return a.operatorBinaryLShift(b);
					if (a.constructor.operatorBinaryLShift) return a.constructor.operatorBinaryLShift(a,b);
					else return a << b;
				},

				binaryRShift(a, b) {
					if (a.operatorBinaryRShift) return a.operatorBinaryRShift(b);
					if (a.constructor.operatorBinaryRShift) return a.constructor.operatorBinaryRShift(a,b);
					else return a >> b;
				},
